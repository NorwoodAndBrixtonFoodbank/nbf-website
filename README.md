...
Database migrations are tracked under /supabase/migrations. Migrations can be handwritten SQL or can be generated by doing a diff between the existing migrations and the schema of the selected database - see https://supabase.com/docs/guides/cli/managing-environments#manual-migration. Before running the migration commands, make sure your supabase is connected to the Dev environment database using `npx supabase link --project-ref <PROJECT_ID>`, where the Dev project ID is visible in the Supabase UI.
...
* Run `npx supabase link --project-ref <PROJECT_ID>` to select the target database
* `npx supbase db push` to apply outstanding migrations

To check they've been applied correctly, either:
* `npx supabase db diff --linked` does a diff
* `npx supabase migration list` on both dev and target databases can be compared

https://supabase.com/docs/guides/cli/managing-environments?environment=ci#deploy-a-migration has guidance on deploying migrations through GitHub Actions.
