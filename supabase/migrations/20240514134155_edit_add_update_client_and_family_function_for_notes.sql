...
BEGIN 
    INSERT INTO clients (notes, full_name, phone_number, address_1, address_2, address_town, address_county, address_postcode, delivery_instructions, dietary_requirements, feminine_products, baby_food, pet_food, other_items, extra_information, flagged_for_attention, signposting_call_required)
    VALUES (clientRecord->>'notes', clientRecord->>'full_name', clientRecord->>'phone_number', clientRecord->>'address_1', clientRecord->>'address_2', clientRecord->>'address_town', clientRecord->>'address_county', clientRecord->>'address_postcode', clientRecord->>'delivery_instructions',array(select * from jsonb_array_elements_text(clientRecord->'dietary_requirements')), array(select * from jsonb_array_elements_text(clientRecord->'feminine_products')),
