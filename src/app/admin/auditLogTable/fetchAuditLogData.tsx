...
export interface AuditLogError {type: "failedAuditLogFetch", logId: string}
export interface AuditLogCountError {type: "failedAuditLogCountFetch" | "nullCount", logId: string}

export const fetchAuditLog = async (supabase: Supabase, startIndex: number,
    endIndex: number): Promise<AuditLogResponse> => {
    const { data, error } = await supabase.from("audit_log").select("*").range(startIndex, endIndex).order("created_at", {ascending: false});
    ...
    export const fetchAuditLogCount = async (supabase: Supabase): Promise<AuditLogCountResponse> => {
        const { count, error } = await supabase.from("audit_log").select("*", {count: "exact", head: true});
