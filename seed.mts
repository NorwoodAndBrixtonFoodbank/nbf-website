...
const possibleOtherItems = ["Garlic", "Ginger", "Chillies", "Spices", "Hot Water Bottles", "Blankets"]

const eventNamesWithNumberData = [
...
async function generateSeed() {
  ...
  await seed.clients(x => x(500, {
    ...
    dietaryRequirements: (ctx) => copycat.someOf(ctx.seed, [0, possibleDietaryRequirements.length], possibleDietaryRequirements),
    feminineProducts: (ctx) => copycat.someOf(ctx.seed, [0, possibleFeminineProducts.length], possibleFeminineProducts),
    petFood: (ctx) => copycat.someOf(ctx.seed, [0, possiblePets.length], possiblePets),
    otherItems: (ctx) => copycat.someOf(ctx.seed, [0, possibleOtherItems.length], possibleOtherItems),
